...
scene.registerAfterRender(function() {
    if(ball.intersectsMesh(goal, false)) {
        ...
        particleSystem.maunalEmitCount = 21;
...
function createScene() {
    ...
    var light = new BABYLON.DirectionalLight("dir01", new BABYLON.Vector3(0, -0.2, 0.2), scene);
    ...
    particleSystem = new BABYLON.ParticleSystem("particle", 2000, scene);
    particleSystem.emmiter = new BABYLON.Vector3(0,0,0);
    particleSystem.minEmitPower = 1;
    particleSystem.maxEmitPower = 3;
    particleSystem.addVelocityGradient(0,2);

    particleSystem.particleTexture = new BABYLON.Texture("images/unnamed (1).png", scene);
...
window.addEventListener("click", function() {
    ...
    if(selectedObject) {
        if(selectedObject.tag == "ball") {
            ...
            var forceDirection = pushDir.scale(-1500);
